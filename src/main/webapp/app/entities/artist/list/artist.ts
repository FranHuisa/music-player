...
export class Artist implements OnInit {
  ...
  readonly isLoading = this.artistService.artistsResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.artistService.artistsResource.headers();
      if (headers) {
  ...
  delete(artist: IArtist): void {
    const modalRef = this.modalService.open(ArtistDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributesFromResponseBody(data: IArtist[]): IArtist[] {
    return data;
  }
